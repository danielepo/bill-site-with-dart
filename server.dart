...
void main() {
  ...
  if (!new Directory(buildPath).existsSync()) {
    log.severe("The 'build/web' directory was not found. Please run 'pub build'.");
    return;
  }
  ...
  HttpServer.bind(InternetAddress.LOOPBACK_IP_V4, port).then((server) {
    ...
    _forwardRequestsToTheHandler(router);    
    var virDir = _initializeVirtualDirectory(buildPath);
...
void _forwardRequestsToTheHandler(router){
// The client will connect using a WebSocket. Upgrade requests to '/ws' and
    // forward them to 'handleWebSocket'.
    router.serve('/ws')
      .transform(new WebSocketTransformer())
      .listen(_handleWebSocket);
}
http_server.VirtualDirectory _initializeVirtualDirectory(buildPath){
  ...
  webSocket
    ...
    .listen((json) {
      ...
      switch (request) {
        case 'addOutgoing':
        case 'addIncoming':
          _addTransaction(json,collection);
...
void _addTransaction(json,collection){
...
void _returnTables(json, WebSocket webSocket){
